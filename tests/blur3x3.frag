...
void main(void)
{
   ...
   vec4 sum = vec4(0.0);
			...
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(-step_w, -step_h));
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(0.0, -step_h)) * 2.0;
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(step_w, -step_h));
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(-step_w, 0.0))*2.0;
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(0.0, 0.0))*4.0;
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(step_w, 0.0))*2.0;
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(-step_w, step_h));
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(0.0, step_h))*2.0;
			sum += texture2D(texture, gl_TexCoord[0].st + vec2(step_w, step_h));
